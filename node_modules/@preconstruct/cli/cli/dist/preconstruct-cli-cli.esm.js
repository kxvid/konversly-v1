...
async function validateIncludedFiles(pkg) {
  try {
    ...
    let result = new Set(packedFilesArr.map(p => path__default.normalize(p)));
...
function retrieveConfigFilenameOrThrow(dirname, ts, configName, pkgName) {
  if (configName !== CONFIGURATION_DEFAULT_TSCONFIG) {
    const customConfigFilename = path__default.resolve(dirname, configName);
...
async function getProgram(dirname, pkgName, ts, configName) {
  ...
  return normalizePath(configFileName) === normalizePath(path__default.join(dirname, configName)) ? nonMemoizedGetProgram(ts, configFileName) : memoizedGetProgram(ts)(configFileName);
}
      ...
      if (!resolvedModule || !resolvedModule.resolvedFileName.startsWith(normalizedPkgDir) || resolvedModule.resolvedFileName.startsWith(normalizedPkgDirNodeModules)) {
        if (resolvedModule && checkTypeDependencies) {
          checkTypeImportDeclaredInDeps(resolvedModule.resolvedFileName, pkg, () => {
            const sourceFile = program.getSourceFile(typescript.sys.useCaseSensitiveFileNames ? filename : filename.toLowerCase());
            return normalizePath(path__default.relative(pkg.directory, sourceFile ? sourceFile.fileName : filename));
          });
        }
        return imported;
      }
  ...
  return {
    ...
    async generateBundle(opts, bundle) {
      ...
      for (const n in bundle) {
        ...
        const baseDtsFilename = path__default.basename(dtsFileName);
...
async function writeDevTSFiles(entrypoint, hasDefaultExport) {
  ...
  let promises = [fs.outputFile(dtsReexportFilename, dtsTemplate(baseDtsFilename, hasDefaultExport, pathToImport, relativePathWithExtension))];
        ...
        for (const conditions of exportsFieldConfig.conditions.groups.keys()) {
          ...
          if (exportsFieldConfig.importConditionDefaultExport === "default") {
            entrypointPromises.push(getHasDefaultExportPromise().then(hasDefaultExport => {
              ...
              return Promise.all([fs.writeFile(filepath, mjsTemplate(
  ...
  return {
    ...
    load(id) {
      if (id.startsWith(internalModulePrefix)) {
        ...
        const name = path__default.basename(innerId).replace(/\.[tj]sx?$/, "").replace(/[^\w]/g, "_");
  ...
  const config = {
    ...
    onwarn: warning => {
      if (typeof warning === "string") {
        warnings.add(`An unhandled Rollup error occurred: ${chalk.red(
        // @ts-ignore
        warning.toString())}`);
        return;
      }
...
async function watchPackage(pkg) {
  ...
  watcher.on("event", event => {
    ...
    switch (event.code) {
      ...
      case "BUNDLE_START":
        {
          info(chalk.cyan(`bundles ${chalk.bold(typeof event.input === "string" ? relativePath(event.input) : Array.isArray(event.input) ? event.input.map(relativePath).join(", ") : event.input === undefined ? "" : Object.values(event.input)
          // @ts-ignore
