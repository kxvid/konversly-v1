...
let limit = pLimit__default["default"](1);
...
class Item {
  constructor(filePath, contents, jsonDataByPath) {
    this.indent = detectIndent__default["default"](contents).indent || "  ";
    this.path = filePath;
    this.directory = path__namespace["default"].dirname(filePath);
    this._jsonDataByPath = jsonDataByPath;
    if (!jsonDataByPath.has(this.path)) {
      const json = parseJson__default["default"](contents, filePath);
  ...
  async save() {
    ...
    if (stringified !== this._jsonDataByPath.get(this.path).stringifiedSaved) {
      await fs__namespace.writeFile(this.path, JSON.stringify(json, null, this.indent) + "\n");
      return true;
    }
    return false;
  }
...
const CONFIGURATION_DEFAULT_TSCONFIG = "tsconfig.json";
...
const conditionsSchema = zod.z.lazy(() => zod.z.union([zod.z.string(), zod.z.null(), zod.z.record(conditionsSchema)]));
const importsSchema = zod.z.record(zod.z.string().startsWith("#"), conditionsSchema).default({});
...
function getFieldsUsedInEntrypoints(descriptors) {
  ...
  for (let descriptor of descriptors) {
    if (descriptor.contents !== undefined) {
      let parsed = parseJson__default["default"](descriptor.contents, descriptor.filename);
  ...
  return Promise.all(descriptors.map(async ({
    ...
    if (pkg.isTypeModule()) {
      if (contents !== undefined && pkg.path !== filename) {
        ...
        await fs__namespace.remove(filename);
        const contents = await fs__namespace.readdir(path__namespace["default"].dirname(filename));
        if (contents.length === 0 || contents.length === 1 && contents[0] === "dist") {
          await fsPromises__default["default"].rm(path__namespace["default"].dirname(filename), {
            recursive: true
          });
        }
      }
      return new Entrypoint(filename, getPlainEntrypointContent(pkg, fields, path__namespace["default"].dirname(filename), pkg.indent), pkg, sourceFile);
    }
...
class Package extends Item {
  get configTsconfig() {
    var _this$json$preconstru, _this$json$preconstru2;
    return (_this$json$preconstru = (_this$json$preconstru2 = this.json.preconstruct) === null || _this$json$preconstru2 === void 0 ? void 0 : _this$json$preconstru2.tsconfig) !== null && _this$json$preconstru !== void 0 ? _this$json$preconstru : CONFIGURATION_DEFAULT_TSCONFIG;
  }
  ...
  static async create(directory, project, isFix) {
    let filePath = path__namespace["default"].join(directory, "package.json");
    let contents = await fs__namespace.readFile(filePath, "utf-8");
    let pkg = new Package(filePath, contents, project._jsonDataByPath);
    pkg.project = project;
    let entrypoints = await fastGlob__default["default"](pkg.configEntrypoints, {
      cwd: path__namespace["default"].join(pkg.directory, "src"),
      onlyFiles: true,
      absolute: true
    });
...
async function validateIncludedFiles(pkg) {
  try {
    ...
    let result = new Set(packedFilesArr.map(p => path__namespace["default"].normalize(p)));
...
class Project extends Item {
  ...
  static async create(_directory, isFix = false) {
    const directory = await fs__namespace.realpath.native(_directory);
    let filePath = path__namespace["default"].join(directory, "package.json");
    let contents = await fs__namespace.readFile(filePath, "utf-8");
  ...
  async _packages(isFix) {
    ...
    let filenames = await fastGlob__default["default"](this.configPackages, {
      cwd: this.directory,
      onlyDirectories: true,
      absolute: true
    });
...
function validatePackage(pkg) {
  ...
  if (pkg.isTypeModule()) {
    keys$1(fields).forEach(field => {
      if (fields[field]) {
        throw new FixableError(`"type": "module" packages should not use the "${field}" field.`, pkg.name);
      }
    });
    return;
  }
  ...
  if (fields["umd:main"]) {
    ...
    for (let depName in pkg.json.dependencies) {
      let depPkgJson;
      try {
        depPkgJson = unsafeRequire$1(resolveFrom__default["default"](pkg.directory, depName + "/package.json"));
      } catch (err) {
...
const isFieldValid = {
  ...
  browser(entrypoint) {
    return equal__default["default"](entrypoint.json.browser, validFieldsForEntrypoint.browser(entrypoint));
  },
  exports(pkg) {
...
const EXPERIMENTAL_FLAGS = new Set(["logCompiledFiles", "keepDynamicImportAsDynamicImportInCommonJS", "importsConditions", "distInRoot", "typeModule", "checkTypeDependencies"]);
...
function loadTypeScript(packageDir, projectDir, pkgName) {
  try {
    return require(resolveFrom__default["default"](packageDir, "typescript"));
  } catch (err) {
...
async function nonMemoizedGetProgram(typescript, configFileName) {
  let configFileContents = await fs__namespace.readFile(configFileName, "utf8");
...
function retrieveConfigFilenameOrThrow(dirname, ts, configName, pkgName) {
  if (configName !== CONFIGURATION_DEFAULT_TSCONFIG) {
    const customConfigFilename = path__namespace["default"].resolve(dirname, configName);
    if (!ts.sys.fileExists(customConfigFilename)) {
      throw new FatalError(`the custom TypeScript config file ${configName} does not exist.`, pkgName);
    }
    return customConfigFilename;
  }
  const configFileName = ts.findConfigFile(dirname, ts.sys.fileExists, configName);
...
async function getProgram(dirname, pkgName, ts, configName) {
  let configFileName = retrieveConfigFilenameOrThrow(dirname, ts, configName, pkgName);
  ...
  return normalizePath__default["default"](configFileName) === normalizePath__default["default"](path__namespace["default"].join(dirname, configName)) ? nonMemoizedGetProgram(ts, configFileName) : memoizedGetProgram(ts)(configFileName);
}
const dtsFileRegex = /\.d\.[cm]?ts$/;
function getDeclarationsForFile(filename, typescript, program, normalizedPkgDir, projectDir, diagnosticsHost, /** This will only be called once per unique module specifier in a file */
visitModuleSpecifier) {
  ...
  if (dtsFileRegex.test(filename)) {
    const magicString = new MagicString__default["default"](sourceFile.text);
    const visitor = node => {
...
function checkTypeImportDeclaredInDeps(resolved, pkg, filename) {
  var _pkg$json$dependencie, _pkg$json$peerDepende;
  const split = normalizePath__default["default"](resolved).split("/");
  ...
  const dep = ((_pkg$json$dependencie = pkg.json.dependencies) === null || _pkg$json$dependencie === void 0 ? void 0 : _pkg$json$dependencie[pkgNameOrScope]) || ((_pkg$json$peerDepende = pkg.json.peerDependencies) === null || _pkg$json$peerDepende === void 0 ? void 0 : _pkg$json$peerDepende[pkgNameOrScope]);
  if (!dep) {
    throw new FatalError(`dependency ${JSON.stringify(pkgNameOrScope)} used by types for ${filename()} is not declared in dependencies or peerDependencies`, pkg.name);
  }
}
function getDeclarationsWithImportedModuleSpecifiersReplacing(typescript, program, normalizedPkgDir, pkg, resolveModuleName, resolvedEntrypointSources) {
    ...
    const handleImport = imported => {
      ...
      if (!resolvedModule || !resolvedModule.resolvedFileName.startsWith(normalizedPkgDir) || resolvedModule.resolvedFileName.startsWith(normalizedPkgDirNodeModules)) {
        if (resolvedModule && checkTypeDependencies) {
          checkTypeImportDeclaredInDeps(resolvedModule.resolvedFileName, pkg, () => {
            const sourceFile = program.getSourceFile(typescript.sys.useCaseSensitiveFileNames ? filename : filename.toLowerCase());
            return normalizePath__default["default"](path__namespace["default"].relative(pkg.directory, sourceFile ? sourceFile.fileName : filename));
          });
        }
        return imported;
      }
  ...
  return {
    ...
    async generateBundle(opts, bundle) {
      ...
      const {
        program,
        options
      } = await getProgram(pkg.directory, pkg.name, typescript, pkg.configTsconfig);
      ...
      const declarations = getDeclarationsWithImportedModuleSpecifiersReplacing(typescript, program, normalizedDirname, pkg, resolveModule, [...entrypointSourceToTypeScriptSource.values()]);
      ...
      for (const n in bundle) {
        ...
        const typeScriptSource = entrypointSourceToTypeScriptSource.get(normalizePath__default["default"](facadeModuleId));
        if (!typeScriptSource) {
          // will happen when only some entrypoints are TypeScript
          continue;
        }
        let dtsFilename = srcFilenameToDtsFilenameMap.get(normalizePath__default["default"](typeScriptSource));
        if (!dtsFilename) {
        ...
        const baseDtsFilename = path__namespace["default"].basename(dtsFileName);
        ...
        const dtsFileSource = dtsTemplate(baseDtsFilename, hasDefaultExport, sourceWithExtension, `${relativeToSource}.d.ts`);
        this.emitFile({
          type: "asset",
...
function replaceTsExtensionWithJs(pathname) {
  return pathname.replace(/\.(?:tsx|([mc])?ts)$/, ".$1js");
}
async function writeDevTSFiles(entrypoint, hasDefaultExport) {
  var _entrypoint$package$e;
  const dtsReexportFilename = path__namespace["default"].join((entrypoint.package.project.experimentalFlags.distInRoot ? entrypoint.package : entrypoint).directory, "dist", getBaseDistName(entrypoint) + (entrypoint.package.isTypeModule() ? "" : ".cjs") + ".d.ts");
  ...
  let promises = [fs__namespace.outputFile(dtsReexportFilename, dtsTemplate(baseDtsFilename, hasDefaultExport, pathToImport, relativePathWithExtension))];
  if (((_entrypoint$package$e = entrypoint.package.exportsFieldConfig()) === null || _entrypoint$package$e === void 0 ? void 0 : _entrypoint$package$e.importConditionDefaultExport) === "default" && !entrypoint.package.isTypeModule()) {
    const dmtsReexportFilename = path__namespace["default"].join(entrypoint.directory, validFieldsForEntrypoint.main(entrypoint)).replace(/\.js$/, ".d.mts");
    const baseDmtsFilename = path__namespace["default"].basename(dmtsReexportFilename);
    promises.push(fs__namespace.outputFile(dmtsReexportFilename, dmtsTemplate(baseDmtsFilename, hasDefaultExport, pathToImport, relativePathWithExtension)));
    if (hasDefaultExport) {
...
async function dev(projectDir) {
  ...
  await Promise.all(project.packages.map(async pkg => {
    const exportsFieldConfig = pkg.exportsFieldConfig();
    let distDirectory = path__namespace["default"].join(pkg.directory, "dist");
    await fs__namespace.remove(distDirectory);
    await fs__namespace.ensureDir(distDirectory);
    return Promise.all(pkg.entrypoints.map(async entrypoint => {
      ...
      if (pkg.isTypeModule() && exportsFieldConfig && exportsFieldConfig.conditions.kind === "imports") {
        for (const conditions of exportsFieldConfig.conditions.groups.keys()) {
          entrypointPromises.push(fs__namespace.symlink(entrypoint.source, path__namespace["default"].join(pkg.directory, getDistFilenameForConditionsWithTypeModule(entrypoint, conditions))));
        }
      ...
      if ((exportsFieldConfig === null || exportsFieldConfig === void 0 ? void 0 : exportsFieldConfig.conditions.kind) === "imports") {
        for (const conditions of exportsFieldConfig.conditions.groups.keys()) {
          ...
          entrypointPromises.push(fs__namespace.symlink(entrypoint.source, path__namespace["default"].join(distRoot, getDistFilenameForConditions(entrypoint, conditions.concat("module")))), fs__namespace.writeFile(path__namespace["default"].join(distRoot, getDistFilenameForConditions(entrypoint, conditions)), cjsTemplate));
          if (exportsFieldConfig.importConditionDefaultExport === "default") {
            entrypointPromises.push(getHasDefaultExportPromise().then(hasDefaultExport => {
              ...
              return Promise.all([fs__namespace.writeFile(filepath, mjsTemplate(
...
async function cleanEntrypoint(entrypoint) {
  ...
  let distDirectory = path__namespace["default"].join(entrypoint.directory, "dist");
  await fs__namespace.remove(distDirectory);
  if (!entrypoint.package.project.experimentalFlags.distInRoot) {
    await fs__namespace.ensureDir(distDirectory);
  }
...
function commonjsRequireHookTemplate(entrypoint) {
  const distDirectory = path__namespace["default"].join((entrypoint.package.project.experimentalFlags.distInRoot ? entrypoint.package : entrypoint).directory, "dist");
  let entrypointPath = normalizePath__default["default"](path__namespace["default"].relative(distDirectory, entrypoint.source));
  return `"use strict";
...
if (process.env.NODE_ENV === "production") {
  module.exports = require("./${path__namespace["default"].basename(getProdPath(mainFieldPath))}");
} else {
  module.exports = require("./${path__namespace["default"].basename(getDevPath(mainFieldPath))}");
}\n`;
        this.emitFile({
          type: "asset",
  ...
  return {
    ...
    async generateBundle(opts, bundle) {
      ...
      for (const n in bundle) {
        ...
        const cjsRelativePath = `./${path__namespace["default"].basename(mjsPath, ".mjs")}.js`;
        this.emitFile({
          type: "asset",
          fileName: mjsPath,
...
let shouldUseWorker = process.env.DISABLE_PRECONSTRUCT_WORKER !== "true" && process.env.NODE_ENV !== "test" && !ciInfo.isCI;
let worker;
let unsafeRequire = require;
function createWorker() {
  if (shouldUseWorker) {
    worker = new Worker__default["default"](require.resolve("@preconstruct/cli/worker"));
  } else {
...
const lru = new QuickLRU__default["default"]({
  maxSize: 1000
});
let extensionRegex = /\.[tj]sx?$/;
let externalHelpersCache = new Map();
const resolvedBabelCore = require.resolve("@babel/core");
const babelHelpers = require(resolveFrom__default["default"](resolvedBabelCore, "@babel/helpers"));
const babelGenerator = require(resolveFrom__default["default"](resolvedBabelCore, "@babel/generator"));
  ...
  return {
    ...
    load(id) {
      ...
      if (helpersSourceDescription === undefined) {
        const result = babelHelpers.get(helperName, undefined,
        // @ts-ignore
        {
          type: "Identifier",
          name: `_${helperName}`
        ...
        if (helperNodes.every(statement => statement.type !== "ExportDefaultDeclaration" && statement.type !== "ExportNamedDeclaration")) {
          const deps = babelHelpers.getDependencies(helperName);
          helpers = `${deps.map(dep => `import ${dep} from "${babelHelpersModuleStart + dep}";`).join("\n")}\n${helpers}\n export { _${helperName} as default };`;
        }
  ...
  return {
    ...
    transform(code, id) {
      if (code.includes("process.env.NODE_ENV")) {
        let magicString = new MagicString__default["default"](code);
        const ast = (() => {
...
function serverComponentsPlugin({
  ...
  return {
    ...
    async resolveId(source, importer, opts) {
      ...
      if (typeof loaded.meta.directivePreservedFile === "string" && importer !== undefined) {
        return {
          id: internalModulePrefix + (loaded.hasDefaultExport ? "1" : "0") + resolved.id
        };
      }
      return resolved;
    },
    load(id) {
      if (id.startsWith(internalModulePrefix)) {
        ...
        const name = path__namespace["default"].basename(innerId).replace(/\.[tj]sx?$/, "").replace(/[^\w]/g, "_");
        const importPath = "__USE_CLIENT_IMPORT__" + referenceId + `__USE_CLIENT_IMPORT__/${name}`;
        return `export * from ${JSON.stringify(importPath)};${hasDefaultExport ? `\nexport { default } from ${JSON.stringify(importPath)};` : ""}`;
      }
      return null;
    },
    transform(code, id) {
    ...
    renderChunk(code, chunk) {
      ...
      if (chunk.facadeModuleId !== null) {
        ...
        const directive = moduleInfo === null || moduleInfo === void 0 ? void 0 : moduleInfo.meta.directivePreservedFile;
        if (directive) {
    ...
    async generateBundle(opts, bundle) {
      for (const n in bundle) {
        ...
        if (!isEntrySourceTypeScript) {
          let flowMode = false;
          let source = await fs__namespace.readFile(file.facadeModuleId, "utf8");
          if (source.includes("@flow")) {
            flowMode = file.exports.includes("default") ? "all" : "named";
          }
          if (flowMode !== false) {
            let flowFileSource = flowTemplate(flowMode === "all", normalizePath__default["default"](relativeToSource));
  ...
  const config = {
    ...
    onwarn: warning => {
      if (typeof warning === "string") {
        warnings.add(`An unhandled Rollup error occurred: ${chalk__default["default"].red(
        // @ts-ignore
        warning.toString())}`);
        return;
      }
...
function getGlobal(project, name) {
  if (project.json.preconstruct.globals !== undefined && project.json.preconstruct.globals[name]) {
    ...
    try {
      let pkgJson = require(resolveFrom__default["default"](project.directory, path__namespace["default"].join(name, "package.json")));
...
function writeOutputFile(outputFile, outputOptions) {
  ...
  if (outputFile.type === "asset") {
    ...
    if (outputOptions.sourcemap && outputFile.map) {
      ...
      if (outputOptions.sourcemap === "inline") {
        ...
        url = `${path__namespace["default"].basename(outputFile.fileName)}.map`;
        writeSourceMapPromise = fs__namespace.outputFile(`${fileName}.map`, outputFile.map.toString());
      }
  ...
  return Promise.all([fs__namespace.outputFile(fileName, source), writeSourceMapPromise]);
}
...
async function watchPackage(pkg) {
  ...
  watcher.on("event", event => {
    ...
    switch (event.code) {
      ...
      case "BUNDLE_START":
        {
          info(chalk__default["default"].cyan(`bundles ${chalk__default["default"].bold(typeof event.input === "string" ? relativePath(event.input) : Array.isArray(event.input) ? event.input.map(relativePath).join(", ") : event.input === undefined ? "" : Object.values(event.input)
          // @ts-ignore
