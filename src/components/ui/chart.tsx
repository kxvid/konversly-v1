...
const ChartTooltipContent = React.forwardRef<HTMLDivElement, any>(
  (props, ref) => {
    ...
    const safePayload: TooltipPayload[] = Array.isArray(payload) ? payload : [];
    ...
    return (
      ...
      >
        ...
        <div className="grid gap-1.5">
          {safePayload.filter(item => typeof item === 'object' && item !== null && ('dataKey' in item || 'name' in item)).map((item, index) => {
            const key = `${nameKey || (item as any).name || (item as any).dataKey || "value"}`;
            const itemConfig = getPayloadConfigFromPayload(config, item, key);
            const indicatorColor = color || ((item as any).payload && typeof (item as any).payload === 'object' && 'fill' in (item as any).payload ? (item as any).payload.fill : undefined) || (item as any).color;
            return (
              <div
                    ...
                    >
                      <div className="grid gap-1.5">
                        ...
                        <span className="text-muted-foreground">
                          {(itemConfig as any)?.label ? (itemConfig as any).label : (item as any).name}
                        </span>
                      </div>
...
>(
  (
    ...
    const { config } = useChart();
    const safePayload: RechartsPrimitive.LegendPayload[] = Array.isArray(payload) ? payload : [];
        ...
        {safePayload.map((item, index) => {
          ...
          return (
            ...
            >
              ...
              {itemConfig && itemConfig.label ? itemConfig.label : item.value || key}
            </div>
          );
        })}
      </div>
